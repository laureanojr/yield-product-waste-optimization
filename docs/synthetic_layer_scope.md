...
The largest bucket in the plant's downtime report corresponds to nothing that happened, and the second-largest true cause is invisible. A maintenance team prioritising off that Pareto works on the wrong thing.

`v_downtime_pareto` reads the same reported codes, so the damage is real inside the project, not just illustrative.
...
**Scrap is valued at full standard cost regardless of when it was rejected.** Real plants charge scrap at cost accumulated to that point. And the cost ratios are invented, so nothing about relative scrap cost is a finding.

**Production scrap here is not the same waste as the real layer's overproduction.** One is product rejected on the line; the other is product that didn't sell. They share a project title, not a definition.
...
Built originally on BigQuery (`sql/synthetic/`); ported to DuckDB so it runs without a cloud account. Both produce identical output.

8,770 batches, 16,029 downtime events, 2,166 inspections, across 21 months of real demand.
